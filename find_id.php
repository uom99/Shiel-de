...
<main class="form-signin ">
  ...
  <form name="loginSbmt" id="loginSbmt" method="post" action="find_id_ok.php">
  <a class="navbar-brand" href="index.php"><img src="imgs/L_logo.png" alt="로고" class="mt-2" aria-current="page" ></a>
    <br><br><br><br>

    <div class="form-check">
      <div class="float-end">
      <label for="p_login">개인<input type="radio" name="find_id_ck" id="p_login" value="person" class="form-check-input find_id_ck" checked> </label>
      &ensp;&ensp;&ensp;&ensp;&ensp;
      <label for="c_login">기업<input type="radio" name="find_id_ck" id="c_login" value="company" class="form-check-input find_id_ck"> </label>
      </div>
    </div>
...
<br>
    <button class="w-100 btn btn-lg btn-primary" type="submit">id 찾기</button>
...
<script>
  $(function(){
    ...
    $('.find_id_ck').change(function() {
      var result = $('input[name="find_id_ck"]:checked').val()

      if (result == 'person') {
        $('#loginSbmt').removeAttr("action");
        $('#loginSbmt').attr("action","find_id_ok.php");
        
        
      } 
      else{
        $('#loginSbmt').removeAttr("action");
        $('#loginSbmt').attr("action","find_cid_ok.php");
      }
