          ...
          <div class="pb-3 mb-0 small lh-sm w-100 h-100">
            <div class="d-flex justify-content-between">
              <div class="btn-group w-100" role="group" aria-label="Basic radio toggle button group">
                <input type="radio" class="btn-check write_ck" value="myboard" name="mywrite" id="mywrite1"
                  autocomplete="off" type='hidden' onclick="location.href='mypage.php'">
                ...
                <input type="radio" class="btn-check write_ck" value="myreply" name="mywrite" id="mywrite2"
                  autocomplete="off" type='hidden' onclick="location.href='mypage_reply.php'">
                ...
                <input type="radio" class="btn-check write_ck" value="mylikes" name="mywrite" id="mywrite3"
                  autocomplete="off" type='hidden' onclick="location.href='mypage_likes.php'" checked>
            ...
            <div class="mylikes">
              <table class="table table-hover mt-1" style="border: 1px solid #ddddda">
                ...
                <?php
                                $sql = "select * from likes where nick='$usernick';";
                                ...
                                $l_sql = "SELECT * FROM likes where nick='$usernick' order by idx desc limit $page_start, $list;";
                                $l_result = mysqli_query($conn,$l_sql);
              while($likes = mysqli_fetch_array($l_result)){


                if(strlen($title)>30){
                  $title=str_replace($board["title"],mb_substr($board["title"],0,30,"utf-8")."...",$board["title"]);
                }
                $bl_sql = "SELECT * FROM board where idx='$likes[idx]';";
                $blesult = mysqli_query($conn,$bl_sql);
                $bl_user = mysqli_fetch_array($blesult);

                $sql5 = "select * from reply where idx='".$board['idx']."'";
                $result5 = mysqli_query($conn,$sql5);
                $rep_count = mysqli_num_rows($result5);
                ?>
                <tr class="read_check" style="cursor:pointer" data-action="read.php?idx=<?=$bl_user['idx']?>">
                  <td style="text-align : center" width="70"><?=$likes['idx'];?></td>
                  <td width="150"><?=$bl_user['category']?></td>
                  <td width="300"><?=$bl_user['title']?><span class="text-danger" style="font-size:14px">[<?=$rep_count?>]</span>
                  </td>
                  <td style="text-align : center" width="200"><?=$bl_user['date'];?></td>
                </tr>
                <?php
            ...
            <ul class="pagination justify-content-center">
              <?php
				    	if ($page <= 1){
				    		...
				    		echo "<li class='page-item'><a class='page-link' href='mypage_likes.php?page=1&type=mylikes'> &laquo; </a></li>";
				    	}
				    	...
				    	if ($page <= 1){
				    		...
				    		$pre = $page - 1;
				    		echo "<li class='page-item'><a class='page-link' href='mypage_likes.php?page=$pre&type=mylikes'> < </a></li>";
				    		
				    	}
				    	
				    	for($i = $block_start; $i <= $block_end; $i++){
				    		if($page == $i){
				    			echo "<li class='page-item active'><a class='page-link' href='mypage_likes.php?page=$i&type=mylikes'> $i </a></li>";
				    		} else {
				    			echo "<li class='page-item'><a class='page-link' href='mypage_likes.php?page=$i&type=mylikes'> $i </a></li>";
				    		}
				    	}
				    	...
				    	if($page >= $total_page){
				    		...
				    		$next = $page + 1;
				    		echo "<li class='page-item'><a class='page-link' href='mypage_likes.php?page=$next&type=mylikes'> > </a></li>";
				    	}
					   	
				    	if($page >= $total_page){
				    		// 빈 값
				    	} else {
				    		echo "<li class='page-item'><a class='page-link' href='mypage_likes.php?page=$total_page&type=mylikes'> &raquo; </a></li>";
				    	}
					?>
            </ul>
          </div>
