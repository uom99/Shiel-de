      ...
      <div class="container w-100 h-100 mt-5 pt-2">
        <div id="board_area"> <br>
          <div class="nav w-100 dropdown mt-1">
            <a href="#" data-bs-toggle="dropdown" role="button" class="fs-5 btn btn-dark w-100 dropdown-toggle"
              id="board_text">전체게시판</a>
            <ul class="blist w-100 dropdown-menu dropdown-menu-dark" id="board_change">
              <li><button class="board_category dropdown-item btn fs-5 " role="button" value="board_all"
                  onclick="location.href='board.php'">전체게시판</button></li>
              <li><button class="board_category dropdown-item btn fs-5 " role="button" value="board_notice"
                  onclick="location.href='b_notice.php'">공지사항</button></li>
              <li><button class="board_category dropdown-item btn fs-5 " role="button" value="board_com_notice"
                  onclick="location.href='b_com_notice.php'">업체공지</button></li>
              <li><button class="board_category dropdown-item btn fs-5 " role="button" value="board_review"
                  onclick="location.href='b_review.php'">리뷰</button></li>
              <li><button class="board_category dropdown-item btn fs-5 " role="button" value="board_question"
                  onclick="location.href='b_question.php'">질문</button></li>
              <li><button class="board_category dropdown-item btn fs-5 " role="button" value="board_life"
                  onclick="location.href='b_life.php'">일상</button></li>
              <li><button class="board_category dropdown-item btn fs-5 " role="button" value="board_humor"
                  onclick="location.href='b_humor.php'">유머</button></li>
              <li><button class="board_category dropdown-item btn fs-5 " role="button" value="board_free"
                  onclick="location.href='b_free.php'">자유</button></li>
            </ul>
          </div>
          ...
          <table class="table table-hover mt-1" style=" border: 1px solid #ddddda">
            ...
            $sql4 = "select * from board where category='공지' order by idx desc limit $page_start, 3"; 
            ...
            <tr class="read_check bg-secondary bg-opacity-25 fw-bold text-red" style="cursor:pointer"
              data-action="read.php?idx=<?=$board['idx']?>">

              <td width="70" style="text-align : center"><?=$board['idx'];?></td>
              <td width="120"><?=$board['category']?></td>
              <td width="300">
                <!--  글 가져오기 -->

                <?=$title?> <span class="text-danger" style="font-size:14px">[<?=$rep_count?>]</span>
              </td>
              <td width="120" style="text-align : center"><?=$board['nick']?></td>
              <td width="200" style="text-align : center"><?=$board['date'];?></td>
              <td width="110" style="text-align : center"><?=$board['hit']; ?></td>
              <td width="110" style="text-align : center"><?=$board['like_count']?></td>
            ...
            <ul class="pagination justify-content-center">
              <?php
				    	if ($page <= 1){
				    		...
				    		echo "<li class='page-item'><a class='page-link' href='board.php?page=1'> &laquo; </a></li>";
				    	}
				    	...
				    	if ($page <= 1){
				    		...
				    		$pre = $page - 1;
				    		echo "<li class='page-item'><a class='page-link' href='board.php?page=$pre'> < </a></li>";
				    	...
				    	if($page >= $total_page){
				    		...
				    		$next = $page + 1;
				    		echo "<li class='page-item'><a class='page-link' href='board.php?page=$next'> > </a></li>";
				    	}
					   	
				    	if($page >= $total_page){
				    		// 빈 값
				    	} else {
				    		echo "<li class='page-item'><a class='page-link' href='board.php?page=$total_page'> &raquo; </a></li>";
				    	}
					?>
            </ul>
          </div>
          ...
          <div id="write_btn">
            <a href="write.php"><button class="btn btn-dark float-end">글쓰기</button></a>
          </div>
